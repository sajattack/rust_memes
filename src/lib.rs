...
pub fn ok_boomer() -> Result<Boomer, ()> {
    Ok(Boomer {})
}

pub fn all_star(i_aint_the_sharpest_tool_in_the_shed: Option<i32>) {
    ...
    if let Some(_body_once_told_me_the_world_was_gonna_roll_me) =
        i_aint_the_sharpest_tool_in_the_shed
    {
        'the_mold: for _she_was_looking_kinda_dumb_with_her_finger_and_her_thumb in
            the_shape_of_an_l_on_her_forehead
        {
            if the_years_start_coming && they_dont_stop_coming
                || fed_to_the_rules && hit_the_ground_running
            {
                let _didnt_make_sense = !live_for_fun;
